...
const config = {
    ...
    options: {
        responsive: true,
        animation: {
            duration: 1600,          // 전체 재생 시간(ms)
            easing: 'easeOutQuart',
            delay: (context) => {
                let d = 0;
                if (context.type === 'data' && context.mode === 'default' && !delayed) {
                    d = context.dataIndex * 180; // 조각별 딜레이
                }
                return d;
            },
    ...
    entries.forEach(function (entry) {
        ...
        const configWithFastText = {
            ...
            options: {
                ...baseOptions,
                animation: {
                    ...baseAnim, //기존 duration/easing 등 전부 유지됨
...
if (!isMobile) {
    observer_adoption = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
...
async function loadData() {
    ...
    data_list.vPetInfo.row.forEach((obj, i) => {
        if (obj.CONT.match("성별 :") || obj.CONT.match("성별:")) {
            ...
            // 이미지 배열 길이/인덱스 불일치 대비(없으면 빈값)
            obj.IMG_URL = (data_list_img.vPetImg && data_list_img.vPetImg.row && data_list_img.vPetImg.row[i])
            ...
            let e = b.split('<')[0].split('(')[0].substring(4).length == 2
                ? b.split('<')[0].split('(')[0].substring(3)
                : b.split('<')[0].split('(')[0].substring(4);

            let f = d.split('<')[0].split('(')[0].substring(4).length == 2
