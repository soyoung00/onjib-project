...
async function init() {
    try {
        ...
        function render() {
            ...
            let navHtml = `<li><a href="#" class="prev">&lt;</a></li>`;
            for (let i = 1; i <= totalPages; i++) {
                const activeClass = (i === currentPage) ? 'active' : '';
                navHtml += `<li><a href="#" class="page-link ${activeClass}" data-page="${i}">${i}</a></li>`;
            }
            navHtml += `<li><a href="#" class="next">&gt;</a></li>`;
            ...
            navUl.querySelector('.prev').onclick = function(e) {
                e.preventDefault();
                if (currentPage > 1) {
                    currentPage--;
                    updateStateAndRender();
            ...
            navUl.querySelector('.next').onclick = function(e) {
                e.preventDefault();
        ...
        if (document.querySelector('.rvs_subMain')) {
            ...
            if (item) {
                ...
                if (backBtn) {
                    backBtn.onclick = () => window.location.href = `page/adoption/reviews.html?page=${savedPage}`;
                }
                
                const swiperWrapper = document.querySelector('.swiper-wrapper');
                if (swiperWrapper) {
                    swiperWrapper.innerHTML = item.photo.map(url => `<div class="swiper-slide"><img src="${url}"></div>`).join('');
                    new Swiper(".mySwiper", { 
                        slidesPerView: 1, 
                        loop: true, 
...
if(location.href.match('reviews_sub')){
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!submitBtn || !displayArea) return;
